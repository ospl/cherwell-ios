...
var myApp = {
	start: function() {
		...
		$(document).on("click", ".aStory", function() {
			loaderHome("out", "left");
			loaderArticle("in", "right");
			$('#content-article').css('background-image', 'url(../img/loading-spinner-1.gif)');
		...
		$('#footer-headlines').click(function() {
			loaderHome("out", "right");
			loaderFitCollege("in", "left");
			initialiseFitCollege();

		});

		$('#header-fit-college').click(function() {
			loaderHome("in", "right");
			loaderFitCollege("out", "left");
			ajaxAbort();
		});

		
		$(document).on("click", ".sectionmore", function() {
		...
		XMLParseAndPrint = function(xml, isMainSection, moreID) { //isMainSection, 1=yes, 2=more
			...
			$(xml).find('section').each(function() {
				...
				if (sectionID == 2) { //ADVERT
					/*
					 $('#insert-test').append('
            <div class="con-item">\ <a href="'+$(this).find('link').text()+'">\ <img src="'+$(this).find('img').text()+'" width="100%" />\ </a>\ </div>
');
					 */
				}
		...
		loadHeadlinesXML = function() {
			...
			$.ajax({
				...
				//url: "http://mytestcherwell.co.nf/frontPage.xml",
				dataType: "xml",
				timeout: 30000,
				success: function(xml) {
		...
		commitVote = function(voteLink) {
			...
			$.ajax({
				...
				success: function(html) {
					...
					}).text("You have voted! Come back next week");
...
/*
			var fitCollegeSwiper = $('.swiper-container').swiper({
				//Your options here:
				mode: 'horizontal',
				loop: false,
			...
			$.ajax({
				...
				success: function(xml) {
					...
					var context = {
						...
						adLink: jFC.competitor[2].img,
						adImg: jFC.competitor[2].college,
						voteButtonDisplay: "block",
						uuid: device.uuid
					};
					$("#content-fit-college").html(template(context));
					$("#footer-fit-college").text('Please vote now').css({
						'background-color': '#006600'
					}); //green
				},
				error: function(xhr) {
		...
		loaderOtherPage = function(inOrOut, leftOrRight) {
			switch (inOrOut) {
			case "in":
				currentView = 'other-page'
				$('#content-other-page').attr('class', 'page ' + leftOrRight);
				$('#header-other-page').attr('class', 'header ' + leftOrRight);
				$('#content-other-page').attr('class', 'page transition center');
				$('#header-other-page').attr('class', 'header transition center');
				break;
			case "out":
				$('#content-other-page').attr('class', 'page transition ' + leftOrRight);
				$('#header-other-page').attr('class', 'header transition ' + leftOrRight);
				break;
			}
		}
		...
		function onPause() {
			...
			$('#content-other-page').attr('class', 'page left');
			...
			$('#footer-fit-college').attr('class', 'footer left');	
		}
